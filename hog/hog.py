...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    digit = 1    
    # "digit is use to activate the first say function"
    while score0 < goal or score1 < goal:
        if player == 0:
            tempscore1 = take_turn(strategy0(score0,score1),score1, dice)
            score0 += tempscore1
            ...
            tempscore2 = take_turn(strategy1(score1,score0),score0, dice)
            score1 += tempscore2
            ...
            player = 0
        if digit == 1:
            express = say(score0,score1)
            digit = 0
        else:
            express2 = express(score0,score1)
            express = express2
    ...
    def announce(score0,score1):
        if who == 0:
           score_I_care = score0
            ...
            score_I_care = score1
        high = previous_high
        gain = score_I_care - previous_score
        if score0 == 0 or score1 == 0 or score_I_care == previous_score:
            return announce_highest(who,high,score_I_care)
        else:                 
            if is_swap(score0,score1):
                if score_I_care == score1:
                   difference_gain = score0 - previous_score
                else:
                    difference_gain = score1 - previous_score
                if gain > high:
                    if gain == 1:
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score1,opponent_score):
        if opponent > score1:
            return 0
        elif opponent < score1:
            return num
        else:
            return num
            
    else:
        if free_bacon(opponent) >= margin:
            return 0
        else:
            return num
